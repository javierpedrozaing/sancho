...
class Essential_Grid_Item_Skin {
    ...
    public function output_item_skin($demo = false, $choosen_skin = 0){
		...
		*/
		...
		if (get_field('check_external_link', @$this->post['ID'])){
			$link_external = get_field('external_link', @$this->post['ID']);
		}else{
			$link = get_permalink(@$this->post['ID']);
		}

		$link_post = get_field('check_external_link', @$this->post['ID']) ? $link_external : $link;
		$target_post =  !empty($link_external) ? '_blank' : '_self';
		#if (!get_field('destacado', @$this->post['ID'])) {
			
		
		if (get_field("type", @$this->post['ID']) == "news") {
			echo '<a href="'.$link_post.' " target="_blank">';
			echo '<li id="'.$li_id.'" class="filterall news-grid filter-articulos filter-the-work tp-esg-item itemtoshow isvisiblenow"> ';		
			echo '<div class="item_notice_plugin">';
			echo '<h1>'.get_the_title(@$this->post['ID']).'</h1>';		
			echo '<p>'.the_field("cuerpo_noticia", @$this->post['ID']).'</p>';		
			echo '</div>';
			echo '</li>';
			echo '</a>';

		}elseif (get_field("type", @$this->post['ID']) == "video") {
				$iframe = get_field('link__video', @$this->post['ID']);
			...
			if($iframe){
					echo '<div class="modal-video-article" id="'.@$this->post['ID'].'">';				
					echo '<div class="video-article">';
					echo '<iframe class="video_grid" id="'.@$this->post['ID'].'" width="560" height="315" src="'.$src_grid.'" frameborder="0" allowfullscreen></iframe>';
					//echo '<span class="close-modal-article close-video-grid ashish" data-url="'.get_permalink(@$this->post['ID']).'"></span>';
					echo '<a class="close-modal-article close-video-grid " href="'.get_permalink(@$this->post['ID']).'"></a>';
					echo '</div>';
					echo '</div>';

				echo '<a href="#">';
				echo '<li id="'.$li_id.'" class="filterall item_grid_hover filter-articulos filter-the-work tp-esg-item itemtoshow isvisiblenow"> ';	
				

				if (!empty($id[0])){
					echo '<div class="container-video-grid">';
					echo '<img src="https://img.youtube.com/vi/'.$id[0].'/hqdefault.jpg">';
					echo '</div>';
	
					echo '<div class="container_hover_grid" id="'.@$this->post['ID'].'">';
					echo '<h1>'.get_field("origen", @$this->post['ID']).'</h1>';	

					echo '<p>'.get_the_title(@$this->post['ID']).'</p>';
					echo '</div>';
				}
        ...
        echo '<a href="'.$link_post.'" target="_blank"  class="link-javier" >';
					...
					if(isset($this->media_sources[$order]) && $this->media_sources[$order] !== '' && $this->media_sources[$order] !== false){ //found entry
						...
						switch($order){
							...
							case 'content-youtube':
								...
								// $echo_media = '<div class="item_grid_hover">
								// <div class="container-video-grid">
								// 	<img src="http://img.youtube.com/vi/' . esc_attr($this->media_sources[$order]) . '/0.jpg">
								// </div>

								// </div>
								// <div class="modal-video-article" id="'.@$this->post['ID'].'">
								// 	<div class="video-article">
								// 		<iframe class="video_grid" id="'.@$this->post['ID'].'" width="560" height="315" src="'.$src_grid.'" frameborder="0" allowfullscreen></iframe>
								// 		<span class="close-modal-article close-video-grid" data-url="'.get_permalink(@$this->post['ID']).'"></span>
								// 	</div>
								// </div>
								// <div class="container_hover_grid" id="'.@$this->post['ID'].'">
								// 	<h1>'.the_field("origen", @$this->post['ID']).'</h1>
								// 	<p>'.the_title(@$this->post['ID']).'</p>
								// </div>
								// ';

								$echo_media = '<div class="esg-media-video item_grid_hover"  data-youtube="'.esc_attr($this->media_sources[$order]).'" width="'.esc_attr($this->video_sizes[$this->video_ratios['youtube']]['width']).'" height="'.esc_attr($this->video_sizes[$this->video_ratios['youtube']]['height']).'" data-poster="'.esc_attr($video_poster_src).'"></div>';
								$is_video = true;
							break;
							case 'vimeo':
							case 'content-vimeo':
		...
		if($link_set_to !== 'none'){
			...
			switch($link_type_link){
				case 'post':
					if($demo === false){
						if($is_post){
							$link_wrapper = '<a href="'.$link_post.'"'.' target="'.$target_post.'"'.'>%REPLACE%</a>';
						}else{
		...
		if($is_iframe == false AND get_field('textos_grilla', @$this->post['ID'])){ //if we are iFrame, no wrapper and no elements in media should be written
			...
			if($cover_type == 'full' && $c_layer > 0 || ($t_layer > 0 || $c_layer > 0 || $b_layer > 0)){
				...
				if($link_set_to == 'cover' && $link_type_link !== 'none')
					$cover_attr =  get_field('textos_grilla', @$this->post['ID']) ? ' data-clickable="on"' : ' data-clickable="on"' ;
					if (get_field('textos_grilla', @$this->post['ID']) ){
						echo '            <div class="esg-entry-cover'.$cover_group_animation.'"'.$cover_group_animation_delay.$cover_attr.'>'."\n\n";
					}else{
						echo '<div class="esg-entry-cover"'.$cover_attr.'>'."\n\n";
					}
     ...
     } // close else post != news

    //}  close if post != destacado
    }
